...
class JSONReader(readJSONBase.JSONReaderBase):
    def parseJsonData(self, data):
        ...
        def loadStep(stepData, lastStepData, candidateItems):
            ...
            oldLosers = set(lastStepData['losers'])
            losers = [l for l in stepData['losers'] if l not in oldLosers]
            # Look at previous step to see who won
            winners = lastStepData['winners']
            step = rcvResult.Step()
        ...
        def loadGraph(data):
            ...
            for step_i in range(1, numRounds):
                step = loadStep(data['rounds'][step_i],
                                data['rounds'][step_i - 1], candidateItems)
                steps.append(step)
            # One last "step" to mark winners (since we only check them in prev
            # round)
            step = loadStep(
                data['rounds'][step_i],
                data['rounds'][step_i],
